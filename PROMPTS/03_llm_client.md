...
**Vanskelighetsgrad:** Høy – bruk gjerne en sterk modell (GPT-4o,
Gemini Pro) på denne.
...
- Systemprompten i skjelettet er et **minimum**. Hva vil DERE legge til for
  ...
  **Vårt svar** (ligger som `SYSTEMPROMPT_TILLEGG` i `backend/llm_client.py`):
  1. Si først hvordan oppgaven er TOLKET. Ved tvetydig notasjon: nevn begge
     tolkningene, velg én og si tydelig hvilken.
  2. Forklar HVORFOR hvert steg gjøres, ikke bare hva – en medstudent skal
     kunne følge forklaringen.
  3. Unngå unødvendig fagsjargong, og forklar fagord første gang.
  4. Ikke regn i hodet: bruk verktøy også til aritmetikk (`calculate`), slik at
     all regning er etterprøvbar.
  5. Får du «feil» tilbake fra et verktøy: rett opp inputen, eller si ærlig fra
     at beregningen ikke lyktes.
  6. Skriv matematikk som LaTeX mellom dollartegn.
...
- Hvor mange tool-calling-runder skal appen maks tillate før den gir opp (for
  ...
  **Vårt svar:** 8 runder (`MAKS_RUNDER`). Vanlige pensumoppgaver trenger 1–3
  verktøykall; resten er slingringsmonn for at modellen skal kunne rette opp en
  syntaksfeil etter en feilmelding. Grensen hindrer evighetsløkker og at
  tokenforbruket løper løpsk. I siste runde sender vi ikke med verktøy, slik at
  modellen må konkludere i stedet for å bli avbrutt uten svar.
...
- Hvis dere limer svar inn fra en nettleser-KI som ikke gir `response.usage`
  ...
  **Vårt svar:** `tokens_brukt` settes til `null`, og frontend skriver «ukjent»
  hvis ett eneste kall manglet tokenrapport. Det samme gjelder kostnaden: uten
  kjent pris for modellen viser vi «ukjent» pluss en advarsel, i stedet for å
  vise 0 og få det til å se gratis ut.
