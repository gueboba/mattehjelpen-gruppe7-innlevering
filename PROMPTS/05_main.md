...
- Hva skal skje hvis `llm_client.solve_task` kaster en feil (f.eks. API
  ...
  **Vårt svar:** begge deler, men adskilt. `feil` inneholder en forklaring på
  norsk med hva brukeren kan gjøre («kvoten er brukt opp – vent litt eller bytt
  modell i .env»), og `teknisk_detalj` inneholder leverandørens egen melding
  til feilsøking. API-nøkler logges eller vises aldri. Responsen har alltid de
  seks feltene frontend forventer, med `validert: false` – appen skal ikke
  kunne se ut som den har svart når den ikke har det.
...
- Hvilken HTTP-statuskode skal brukes ved ulike feil (f.eks. 400 for dårlig
  ...
  **Vårt svar:** 400 ved tom eller altfor lang oppgave (over 2000 tegn), 500
  når appen selv er feil satt opp (manglende `API_KEY`) eller ved uventede
  feil, 502 når leverandøren svarer med feil (ugyldig nøkkel, ukjent modell,
  mislykket verktøykall), 503 ved oppbrukt kvote eller rate limit, og 504 ved
  tidsavbrudd. Frontend viser innholdet uansett statuskode, slik at brukeren
  får se begrunnelsen og ikke bare en tom side.
